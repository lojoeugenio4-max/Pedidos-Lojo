...
// "Modo rápido": cuando un pedido tiene muchas bolas de Bingo pendientes
// (más de 3), esperar la vuelta completa del bombo por cada una hace el
// proceso desesperantemente lento para el cajero y el cliente. En ese caso
// se usan estos tiempos, muchísimo más cortos, en vez de los de arriba: la
// bola prácticamente aparece directamente, sin mezcla larga ni necesidad de
// pulsar nada entre bola y bola.
const FAST_MIX_SECONDS = 0.12;
const FAST_STOP_SECONDS = 0.08;
const FAST_EXTRACTION_SECONDS = 0.22;
const FAST_LANDING_PAUSE_SECONDS = 0.05;
const FAST_IDLE_SECONDS = 0.35;
...
function PremioCard({ premio }) {
  return (
    <div className={`pro-prize-card ${premio.special ? "pro-prize-card--special" : ""}`}>
...
// Premios normales (Línea, Bingo): columna a la izquierda del bombo.
function PremiosLaterales({ premios }) {
  const activos = (premios || []).filter((premio) => premio?.active && !premio?.special);
  if (!activos.length) return null;
  return (
    <aside className="pro-prizes-col" aria-label="Premios del Bingo">
      {activos.map((premio, index) => (
        <PremioCard premio={premio} key={index} />
      ))}
    </aside>
  );
}

// Premios especiales (Línea especial, Bingo especial): fila centrada
// justo encima del bombo, para que destaquen más que los normales.
function PremiosEspeciales({ premios }) {
  const activos = (premios || []).filter((premio) => premio?.active && premio?.special);
  if (!activos.length) return null;
  return (
    <div className="pro-special-row" aria-label="Premios especiales del Bingo">
      {activos.map((premio, index) => (
        <PremioCard premio={premio} key={index} />
      ))}
    </div>
...
export default function BingoDrumStage({
  ...
  const revealNumber = useCallback((number) => {
    ...
    const mixS = fastMode ? FAST_MIX_SECONDS : MIX_SECONDS;
    const stopS = fastMode ? FAST_STOP_SECONDS : STOP_SECONDS;
    const extractionS = fastMode ? FAST_EXTRACTION_SECONDS : EXTRACTION_SECONDS;
    const landingS = fastMode ? FAST_LANDING_PAUSE_SECONDS : LANDING_PAUSE_SECONDS;
    const idleMs = fastMode ? FAST_IDLE_SECONDS * 1000 : 3900;

    setPendingNumber(number);
    setPhase("starting");
    // En modo rápido no suena el ronroneo largo del motor (sonaría cortado
    // y machacón bola tras bola): solo se mantienen los sonidos breves de
    // caída y revelado, que sí caben en el tiempo tan corto.
    if (!fastMode) audioCue("motor");
    schedule(() => setPhase("spinning"), fastMode ? 40 : 560);
    schedule(() => setPhase("stopping"), mixS * 1000);
    schedule(() => setPhase("extracting"), (mixS + stopS) * 1000);
    schedule(() => {
      setPhase("landed");
      audioCue("drop");
    }, (mixS + stopS + extractionS) * 1000);
    schedule(() => {
      setLocalNumbers((current) => (current.includes(number) ? current : [...current, number]));
      setPhase("reveal");
      audioCue("reveal");
      window.setTimeout(() => cantarNumero(number), fastMode ? 40 : 260);
      onRevealComplete?.(number);
      schedule(() => setPhase("idle"), idleMs);
    }, (mixS + stopS + extractionS + landingS) * 1000);
  }, [schedule, onRevealComplete, fastMode]);
  ...
  return (
    <main className={`pro-stage pro-stage--${phase}`}>
      ...
      <section className={`pro-content ${premios?.length ? "has-prizes" : ""}`}>
        <PremiosLaterales premios={premios} />

        <div className="pro-machine-col">
          <PremiosEspeciales premios={premios} />
          ...
          {mostrarControles && (
            <div className="pro-draw-control">
              ...
              <span>
                {drawMessage ||
                  (fastMode
                    ? "Muchas tiradas: salen solas, sin pulsar nada."
                    : "Pulsa para extraer la bola. Se verá a la vez en la pantalla grande.")}
              </span>
            </div>
          )}
